    ...
    NFTMetadata
{
   ...
   */
    ...
    /// @dev Lets anyone mint an NFT. The `onlyRole(MINTER_ROLE)` modifier has been removed.
    function mintTo(address _to, string calldata _uri) external nonReentrant returns (uint256) {
        // `_mintTo` is re-used. The minter role check has been removed to allow public minting.
